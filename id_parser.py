...
# Ignore common stock/watermark lines
...
def _is_date(s: str) -> bool:
    return bool(_DATE_RE.fullmatch(s.strip()) or _DATE_RE.search(s.strip()))

def _looks_like_id(s: str) -> bool:
    if s.strip().startswith("+"):   # likely a phone
        return False
    return bool(re.fullmatch(r"[0-9][0-9\s\-/.]{6,}", s.strip()))

def tidy_text(text: str) -> str:
    """Normalize colons and join dangling value lines with preceding labels."""
...
# id_parser.py — replace _merge_header_school() with this tighter version
def _merge_header_school(lines: List[str]) -> str:
    """
    Merge consecutive ALL-CAPS lines prior to the 'ID CARD' / 'IDENTIFICATION CARD' line.
    Example: ["FAUGET HIGH", "SCHOOL"] -> "FAUGET HIGH SCHOOL"
    """
    header = []
    seen_card = False
    for ln in lines:
        ...
        if ln.isupper() and re.search(r"[A-Z]", ln):
            ...
            # reset if a non-header interrupts
            header = []
    if header:
        s = " ".join(header).strip()
...
def parse_id_fields(text: str) -> Dict[str, str]:
    """
    Robust, line-first parser:
    - Handles 'labels block' then 'values block' (Vision).
    - Type-checks for DOB vs ID and can swap if needed.
    - Ignores watermark/stock lines.
    """
    out: Dict[str, str] = {}
    ...
    # Pass 1: collect labeled pairs and a queue of labels without same-line values
    pending: List[str] = []
    values: List[str] = []
    i = 0
    while i < N:
        ...
        for key, rx in LABEL_RE.items():
            ...
            if m:
                ...
                break
        if not matched_key:
            # potential value line (for a previous pending label block)
            values.append(ln)
        i += 1

    # Pass 2: if we saw a block of labels followed by block of values, pair by order
    # Only use values that aren't themselves labels
    clean_values = [v for v in values if not any(rx.match(v) for rx in LABEL_RE.values()) and v != ":"]
    # Type-aware assignment with 1-step lookahead for DOB
    vi = 0
    for key in pending:
        if vi >= len(clean_values):
            break
        v = clean_values[vi].strip()
        # If key is dob but v is not a date and next value is a date, swap
    ...
    if "website" not in out:
        m = (re.search(r"(?:https?://)?(?:www\.|WWW)[A-Za-z0-9\-.]+\.[A-Za-z]{2,}(?:/[^\s]*)?", blob)
    ...
    # Disambiguate ID vs phone only if labels were missing:
